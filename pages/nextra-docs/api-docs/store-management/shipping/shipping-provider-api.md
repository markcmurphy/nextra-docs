...
``` title="Example carrier connection request with connection settings" lineNumbers
...
```
&nbsp;
``` title="Example carrier connection request without connection settings" lineNumbers
POST https://example.com/shipping/carrier/connection
X-Auth-Token: {{ACCESS_TOKEN}}
Content-Type: application/json
Accept: application/json
...
``` title="Example request: Create a shipping method in a specified zone" lineNumbers
...
``` title="Example request: Shipping rates" lineNumbers
